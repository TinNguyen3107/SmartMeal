...
export const RecipeExplorer: React.FC<RecipeExplorerProps> = ({
  ...
  useEffect(() => {
    async function loadRecipes() {
      try {
        ...
        if (!res.ok) throw new Error(data.message || 'Không thể tải kho công thức.');
        setRecipes(data.recipes || []);
      } catch (err) {
        console.error('Failed to load recipes:', err);
        setLoadError(err instanceof Error ? err.message : 'Không thể tải kho công thức.');
      } finally {
        setIsLoading(false);
      }
    }
  ...
  const categories = ['All', 'Món chính', 'Canh / Súp', 'Món xào', 'Món kho', 'Món chiên / nướng', 'Salad / Khai vị', 'Món ăn nhanh'];
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      {/* Filter & Search Bar */}
      <div className="bg-white border border-[#EAE7E0] rounded-[32px] p-6 card-shadow space-y-4">
        {/* Search */}
      ...
      {/* Grid of Recipes */}
      {loadError && (
        <div className="rounded-[28px] border border-[#C87D55]/30 bg-[#D9AE94]/15 p-5 text-sm font-semibold text-[#8C5D36]">
          {loadError}
        </div>
      )}

      {isLoading && (
        <div className="rounded-[28px] border border-[#EAE7E0] bg-white p-8 text-center text-sm font-semibold text-[#7D857E] card-shadow">
          Đang tải kho công thức...
        </div>
      )}

      {!isLoading && !loadError && filteredRecipes.length === 0 && (
        <div className="rounded-[28px] border border-[#EAE7E0] bg-white p-8 text-center card-shadow">
          <SlidersHorizontal className="mx-auto mb-3 h-6 w-6 text-[#8BA08E]" />
          <p className="font-serif text-lg font-bold text-[#3D3D3D]">Chưa tìm thấy công thức phù hợp</p>
          <p className="mx-auto mt-2 max-w-md text-sm leading-relaxed text-[#7D857E]">Thử thay đổi từ khóa, chế độ ăn hoặc xóa bộ lọc để xem lại toàn bộ kho công thức.</p>
          <button
            type="button"
            onClick={resetFilters}
            className="mt-5 rounded-full bg-[#8BA08E] px-5 py-2.5 text-xs font-bold text-white transition-colors hover:bg-[#798E7C]"
          >
            Xóa bộ lọc
          </button>
        </div>
      ...
      {!isLoading && !loadError && filteredRecipes.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          return (
            <div
              key={recipe.id}
              className="bg-white border border-[#EAE7E0] hover:border-[#D1CEC7] rounded-[32px] overflow-hidden card-shadow flex flex-col group transition-all"
            >
