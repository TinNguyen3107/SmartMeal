...
export const RecommendationHub: React.FC<RecommendationHubProps> = ({
  ...
  const [nutritionGoal, setNutritionGoal] = useState<NutritionGoal>(userPreferences?.nutritionGoal || 'BALANCED');

  useEffect(() => {
    if (!userPreferences) return;
    setSelectedDietTags(userPreferences.dietaryTypes || ['Vietnamese']);
    setMaxCookingTime(userPreferences.maxCookingTime || 35);
    setDifficultyFilter(userPreferences.preferredDifficulty || 'Any');
    setNutritionGoal(userPreferences.nutritionGoal || 'BALANCED');
  }, [userPreferences]);

  // Results state
  const [isLoading, setIsLoading] = useState(false);
  const [recommendations, setRecommendations] = useState<RecommendedRecipe[]>([]);
  const [generatedDraft, setGeneratedDraft] = useState<RecipeDraft | null>(null);
    ...
    try {
      const payload = {
        ...
        preferences: {
          ...
          semanticQuery: `${nutritionGoal} ${selectedDietTags.join(' ')}`
        }
      };
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.message || 'Không thể lấy gợi ý món ăn.');
      ...
      setShowGeneratedDraft(receivedRecommendations.length === 0 && Boolean(data.generatedDraft));
    } catch (err) {
      console.error('Recommendation API error:', err);
      setRecommendations([]);
      setGeneratedDraft(null);
      setApiError(err instanceof Error ? err.message : 'Không thể lấy gợi ý món ăn.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="bg-[#4A5D4E] text-white rounded-[36px] p-8 sm:p-10 card-shadow-lg relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-3xl">
          ...
          <p className="mt-3 text-[#E9EDC9] text-sm sm:text-base leading-relaxed opacity-95">
            Nhập nguyên liệu bằng tìm kiếm, danh mục hoặc câu mô tả tự nhiên.
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        {/* LEFT COLUMN: Input Methods & Selected Ingredients Tray (5 Cols) */}
        <div className="lg:col-span-5 space-y-6">
          {/* Input Method Switcher Box */}
          <div className="bg-white border border-[#EAE7E0] rounded-[32px] p-6 card-shadow">
            ...
            <div className="grid grid-cols-3 gap-1 p-1.5 bg-[#F2EDE4] rounded-2xl border border-[#EAE7E0] text-xs mb-5">
              <button
                ...
                className={`py-2 rounded-xl font-semibold transition-all flex flex-col items-center gap-1 ${
                  inputTab === 'search'
            ...
            {/* TAB 3: Natural Language Parser (FR-05 Method 3) */}
            {inputTab === 'nlp' && (
              <div className="space-y-3">
                <p className="text-xs text-[#686868] leading-relaxed">
                  Gõ hoặc dán một câu mô tả tự nhiên. Hệ thống sẽ trích xuất danh sách nguyên liệu và giới hạn thời gian nấu:
                </p>
                <textarea
                ...
                >
                  {isExtractingNlp ? (
                    <>
                      ...
                      Đang phân tích câu mô tả...
                    </>
                  ) : (
                    <>
                      <Sparkles className="w-4 h-4 text-[#D9AE94]" />
                      Phân tích câu tự nhiên & Gợi ý ngay
                    </>
                  )}
                </button>
              </div>
            ...
            <div className="mt-5 pt-4 border-t border-[#F2EDE4] flex flex-col gap-3">
              ...
              {showFilterDrawer && (
                <div className="p-4 rounded-2xl bg-[#F9F7F2] border border-[#EAE7E0] space-y-4 text-xs">
                    ...
                    <div className="grid grid-cols-4 gap-1">
                      {['Any', 'Easy', 'Medium', 'Hard'].map(d => (
                        <button
                          ...
                          className={`py-1.5 rounded-xl font-semibold border text-center transition-colors ${
                            difficultyFilter === d
                  ...
                  <div>
                    <span className="block font-semibold text-[#3D3D3D] mb-1.5">Mục tiêu người dùng:</span>
        ...
        <div className="lg:col-span-7 space-y-6">
            ...
            <section className="bg-[#FDFBF7] border border-[#D8DFB0] rounded-[28px] overflow-hidden card-shadow">
              <div className="p-5 sm:p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="min-w-0">
                  <div className="inline-flex items-center gap-1.5 rounded-full bg-[#E9EDC9] px-2.5 py-1 text-[10px] font-bold text-[#344C39]">
                    <Sparkles className="w-3.5 h-3.5" />
                    CÔNG THỨC LINH HOẠT - CHƯA KIỂM DUYỆT
                  </div>
                  <h3 className="mt-2 font-serif text-lg font-bold text-[#3D3D3D]">{generatedDraft.vietnameseName}</h3>
                  <p className="mt-1 text-xs leading-relaxed text-[#686868]">
                    Được hệ thống dựng từ nguyên liệu bạn vừa nhập. Dùng như một gợi ý cá nhân, không phải công thức do admin xác thực.
                  </p>
                </div>
                <button
                  type="button"
                  onClick={() => setShowGeneratedDraft(value => !value)}
                  className="shrink-0 inline-flex items-center justify-center gap-1.5 rounded-full bg-[#4A5D4E] px-4 py-2.5 text-xs font-bold text-white transition-colors hover:bg-[#344C39]"
                >
                  {showGeneratedDraft ? 'Thu gọn' : 'Xem công thức linh hoạt'}
                  <ArrowRight className={`w-3.5 h-3.5 transition-transform ${showGeneratedDraft ? 'rotate-90' : ''}`} />
                </button>
              </div>

              {showGeneratedDraft && (
                <div className="border-t border-[#D8DFB0] bg-white p-5 sm:p-6">
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-5 text-[11px]">
                    <span className="rounded-xl border border-[#EAE7E0] bg-[#F9F7F2] px-3 py-2 text-center font-semibold text-[#4A5D4E]">
                      {formatMinutes(generatedDraft.preparationTime + generatedDraft.cookingTime)}
                    </span>
                    <span className="rounded-xl border border-[#EAE7E0] bg-[#F9F7F2] px-3 py-2 text-center font-semibold text-[#4A5D4E]">
                      {generatedDraft.estimatedNutrition?.calories ?? generatedDraft.calories ?? '-'} kcal ước tính
                    </span>
                    <span className="rounded-xl border border-[#EAE7E0] bg-[#F9F7F2] px-3 py-2 text-center font-semibold text-[#4A5D4E]">
                      {generatedDraft.estimatedNutrition?.protein ?? '-'}g protein
                    </span>
                    <span className="rounded-xl border border-[#EAE7E0] bg-[#F9F7F2] px-3 py-2 text-center font-semibold text-[#8C5D36]">
                      {generatedDraft.servings} khẩu phần
                    </span>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                      <h4 className="mb-2 flex items-center gap-1.5 text-xs font-bold text-[#3D3D3D]">
                        <Layers className="w-4 h-4 text-[#8BA08E]" /> Nguyên liệu dùng
                      </h4>
                      <ul className="space-y-2 text-xs text-[#3D3D3D]">
                        {generatedDraft.ingredients.map((ingredient, index) => (
                          <li key={`${ingredient.name}-${index}`} className="flex items-center justify-between gap-3 border-b border-[#F2EDE4] pb-2 last:border-0">
                            <span>{ingredient.name}{ingredient.isOptional ? ' (tùy chọn)' : ''}</span>
                            <span className="shrink-0 font-semibold text-[#4A5D4E]">{ingredient.quantity} {ingredient.unit}</span>
                          </li>
                        ))}
                      </ul>
                    </div>
                    <div>
                      <h4 className="mb-2 flex items-center gap-1.5 text-xs font-bold text-[#3D3D3D]">
                        <ChefHat className="w-4 h-4 text-[#8BA08E]" /> Các bước thực hiện
                      </h4>
                      <ol className="space-y-2 text-xs leading-relaxed text-[#3D3D3D]">
                        {generatedDraft.instructions.map(step => (
                          <li key={step.stepNumber} className="flex gap-2">
                            <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-[#E9EDC9] text-[10px] font-bold text-[#344C39]">{step.stepNumber}</span>
                  ...
                  <div className="mt-5 rounded-2xl border border-[#F3C1A8] bg-[#FBE8DE] px-3.5 py-3 text-[11px] leading-relaxed text-[#8C5D36]">
                    Định lượng và dinh dưỡng chỉ là ước tính. Với dị ứng, bệnh lý, giảm cân nghiêm ngặt hoặc chế độ tập luyện, hãy đối chiếu nguyên liệu và khẩu phần trước khi áp dụng.
                  </div>
                </div>
              )}
            </section>
          ...
          {isLoading ? (
            <div className="text-center py-16 bg-white rounded-[32px] border border-[#EAE7E0] card-shadow">
            ...
            <div className="text-center py-16 bg-white rounded-[32px] border border-[#EAE7E0] p-6 card-shadow">
              <AlertCircle className="w-12 h-12 text-[#C87D55] mx-auto mb-3" />
              <h3 className="font-serif text-lg font-bold text-[#3D3D3D]">Không thể tải gợi ý</h3>
              <p className="text-xs text-[#7D857E] max-w-md mx-auto mt-1 mb-4 leading-relaxed">{apiError}</p>
              <button
                onClick={() => runRecommendation()}
            ...
            <div className="space-y-4">
              {filteredRecommendations.map((item, idx) => {
                const { recipe, matchScore, nutritionScore, semanticScore, estimatedNutrition, scoreBreakdown, status, matchedIngredients, missingIngredients, explanation } = item;
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-start gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex flex-wrap items-center gap-3 text-xs text-[#7D857E] mb-3">
                            ...
                            {typeof nutritionScore === 'number' && (
                              <span className="px-2.5 py-0.5 rounded-full bg-[#E9EDC9] text-[10px] font-semibold text-[#4A5D4E]">
                                Dinh dưỡng {nutritionScore}%
                              </span>
                            )}
                            {typeof semanticScore === 'number' && (
                              <span className="px-2.5 py-0.5 rounded-full bg-[#F2EDE4] text-[10px] font-semibold text-[#8C5D36]">
                          ...
                          {estimatedNutrition && (
                            <div className="grid grid-cols-4 gap-1.5 mb-3 text-[10px]">
                              <span className="rounded-xl bg-[#F9F7F2] border border-[#EAE7E0] px-2 py-1 text-center font-semibold text-[#4A5D4E]">
                                {estimatedNutrition.calories} kcal
                              </span>
                              <span className="rounded-xl bg-[#F9F7F2] border border-[#EAE7E0] px-2 py-1 text-center font-semibold text-[#4A5D4E]">
                                {estimatedNutrition.protein}g P
                              </span>
                              <span className="rounded-xl bg-[#F9F7F2] border border-[#EAE7E0] px-2 py-1 text-center font-semibold text-[#8C5D36]">
                                {estimatedNutrition.carbs}g C
                              </span>
                              <span className="rounded-xl bg-[#F9F7F2] border border-[#EAE7E0] px-2 py-1 text-center font-semibold text-[#8C5D36]">
                                {estimatedNutrition.fat}g F
                              </span>
                            </div>
                          )}
                              ...
                              <div className="flex flex-wrap items-center gap-1.5 pt-1.5 border-t border-[#EAE7E0]">
                                ...
                                {missingIngredients.map(miss => (
                                  <button
                                    ...
                                    title="Mở tủ lạnh để cập nhật nguyên liệu"
                                  >
                            ...
                            <div>
                              ...
                              {scoreBreakdown && (
                                <div className="mt-2 grid grid-cols-2 sm:grid-cols-4 gap-1.5">
                                  <span>Match {scoreBreakdown.ingredientMatch}%</span>
                                  <span>Dinh dưỡng {scoreBreakdown.nutritionFit}%</span>
                                  <span>Ngữ nghĩa {scoreBreakdown.semanticSimilarity}%</span>
                                  <span>Sở thích {scoreBreakdown.userPreference}%</span>
                                </div>
                              )}
                            </div>
                          </div>
                        ...
                        <div className="flex items-center gap-2 text-xs text-[#7D857E]">
                          ...
                          <button
                            ...
                            className={`p-2 rounded-full border transition-colors ${
                              feedbackGiven[recipe.id] === 'HELPFUL'
                                ? 'bg-[#8BA08E] text-white border-[#8BA08E]'
                          ...
                          <button
                            ...
                            className={`p-2 rounded-full border transition-colors ${
                              feedbackGiven[recipe.id] === 'NOT_RELEVANT'
