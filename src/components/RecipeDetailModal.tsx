    ...
    async function loadRecipe() {
      try {
        ...
        if (!res.ok || !data.recipe) {
          throw new Error(data.message || 'Không thể tải công thức chi tiết.');
        }
        setRecipe(data.recipe);
        setReviews(data.reviews || []);
      } catch (err) {
        console.error('Failed to load recipe:', err);
        setLoadError(err instanceof Error ? err.message : 'Không thể tải công thức chi tiết.');
      } finally {
        setIsLoading(false);
      }
    }
  ...
  if (loadError || !recipe) {
    return (
      <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
        <div className="max-w-sm w-full rounded-[28px] border border-[#EAE7E0] bg-white p-8 text-center text-[#3D3D3D] card-shadow-lg">
          <AlertCircle className="mx-auto mb-3 h-8 w-8 text-[#C87D55]" />
          <h2 className="font-serif text-xl font-bold">Không thể mở công thức</h2>
          <p className="mt-2 text-sm leading-relaxed text-[#7D857E]">{loadError || 'Công thức này không còn khả dụng.'}</p>
          <button
            type="button"
            onClick={onClose}
      ...
      <div className="bg-white border border-[#EAE7E0] rounded-[36px] max-w-4xl w-full max-h-[92vh] overflow-y-auto card-shadow-lg text-[#3D3D3D] relative">
          ...
          <div className="absolute bottom-6 left-8 right-8 flex flex-col sm:flex-row sm:items-end justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className={`px-4 py-2 rounded-full border flex items-center gap-1.5 text-xs font-bold transition-colors backdrop-blur-sm ${
                  isFavorite
        ...
        <div className="p-6 sm:p-8 space-y-8">
          ...
          <div className="pt-6 border-t border-[#EAE7E0] space-y-6">
            ...
            <div className="space-y-3">
              {reviews.map(rev => (
                <div key={rev.id} className="p-5 rounded-[24px] bg-[#F9F7F2] border border-[#EAE7E0] text-xs space-y-2">
                  <div className="flex items-center justify-between">
